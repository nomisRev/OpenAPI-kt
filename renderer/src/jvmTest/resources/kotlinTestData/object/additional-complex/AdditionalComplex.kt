  ...
  public object Serializer : KSerializer<AdditionalComplex> {
    ...
    override fun serialize(encoder: Encoder, `value`: AdditionalComplex) {
      ...
      value.additional?.forEach { (key, additionalValue) ->
        content[key] = json.encodeToJsonElement(AdditionalProperties.serializer(), additionalValue)
      }
    ...
    override fun deserialize(decoder: Decoder): AdditionalComplex {
      ...
      val additional = (element - knownNames)
        .mapValues { (_, jsonElement) -> json.decodeFromJsonElement(AdditionalProperties.serializer(), jsonElement) }
        .ifEmpty { null }
