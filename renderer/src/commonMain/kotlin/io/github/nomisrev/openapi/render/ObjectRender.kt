...
fun Model.Object.render(
    ...
    when (properties.size) {
        ...
        1 -> {
            ...
            if (additional is Model.Object.AdditionalProperties.Allowed && !additional.value) {
                valueClass(parentClass, baseProperties)
            } else {
                dataClass(parentClass, baseProperties)
            }
        }
...
private fun Model.Object.additionalProperty(): String? =
    when (val additional = additionalProperties) {
        is Model.Object.AdditionalProperties.Allowed -> if (additional.value) {
            ctx.import(TypeName.JsonObject)
            "val additional: JsonObject? = null"
        } else null

        is Model.Object.AdditionalProperties.Schema -> {
            import(additional.value)
            val typeName = additional.value.toTypeName().type()
...
private fun Model.Object.needsSerializer(): Boolean = when (val additional = additionalProperties) {
    is Model.Object.AdditionalProperties.Allowed -> additional.value
                ...
                indented {
                    ...
                    when (val additional = additionalProperties) {
                        ...
                        is Model.Object.AdditionalProperties.Schema -> {
                            ...
                            +"${ctx.indent}put(key, json.encodeToJsonElement(${additional.value.serializer()}, value))"
                            +"}"
                        }
                    }
            ...
            indented {
                ...
                when (val additional = additionalProperties) {
                    ...
                    is Model.Object.AdditionalProperties.Schema -> {
                        ...
                        +"${ctx.indent}.mapValues { (_, value) -> json.decodeFromJsonElement(${additional.value.serializer()}, value) }"
                    }
                }
            }
            +")"
