...
suspend fun ResolvedSchema.anyOfNullable(context: SchemaContext, anyOf: List<ReferenceOr<Schema>>) : Model {
    val schema = anyOf.map { it.resolve(name, context) }.single { it.schema.type != Schema.Type.Basic.Null }
