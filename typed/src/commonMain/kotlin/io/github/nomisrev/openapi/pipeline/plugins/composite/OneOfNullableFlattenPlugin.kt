...
import io.github.nomisrev.openapi.transformers.buildOneOf

object OneOfNullableFlattenPlugin : SchemaTransformerPlugin {
    override val key: PluginKey = PluginKeys.OneOfNullable
    ...
    override suspend fun transform(
        ...
        if (!schema.isOneOfNullableType()) return null

        schema.flattenNull(schema.schema.oneOf!!) { nonNullSchemas ->
            if (nonNullSchemas.size == 1) {
                schema.flattenedSingleNullableBranch(nonNullSchemas.single(), context)
            } else {
                schema.buildOneOf(context, nonNullSchemas)
            }
        }
    }
}
