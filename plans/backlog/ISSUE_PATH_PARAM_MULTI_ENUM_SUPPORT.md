...
Phase 5 moved path-parameter flattening decisions into the typed layer, but inline path unions with more than one enum case still fail during route resolution with:
...
3. The typed layer now owns overload eligibility, so this limitation should also be addressed there instead of remaining as a permanent guardrail.

## Current Behavior

1. `parsePathSegments(...)` emits `PathSegment.OverloadedParameter` only for simple inline unions with at most one enum case.
...
2. If two enum cases erase to the same Kotlin shape, should the typed layer deduplicate them or keep both and rely on renderer naming?
...
1. Specs with inline path `oneOf` unions containing multiple enum cases no longer fail during route resolution by default.
