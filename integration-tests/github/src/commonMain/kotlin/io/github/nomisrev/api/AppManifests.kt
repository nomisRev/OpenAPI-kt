...
import io.github.nomisrev.model.Enterprise
...
interface AppManifests {
    val conversions: AppManifests.Conversions

    interface Conversions {
        @Serializable
        data class AppsCreateFromManifestResponse(
            val id: Long,
            ...
            sealed interface Owner {
                ...
                object Serializer : KSerializer<Owner> {
                    ...
                    override val descriptor: SerialDescriptor =
                        buildSerialDescriptor("io.github.nomisrev.api.AppManifests.Conversions.AppsCreateFromManifestResponse.Owner", PolymorphicKind.SEALED) {
        ...
        sealed interface AppsCreateFromManifestResult {
            data class Created(val value: AppsCreateFromManifestResponse) : AppsCreateFromManifestResult

            data class NotFound(val value: BasicError) : AppsCreateFromManifestResult

            data class UnprocessableEntity(val value: ValidationErrorSimple) : AppsCreateFromManifestResult
        }

        suspend fun appsCreateFromManifest(
            code: String,
        ): AppsCreateFromManifestResult
    }
}

internal class KtorAppManifests(private val client: HttpClient) : AppManifests {
    override val conversions: AppManifests.Conversions = KtorAppManifestsConversions(client)
}

internal class KtorAppManifestsConversions(private val client: HttpClient) : AppManifests.Conversions {
    override suspend fun appsCreateFromManifest(code: String): AppManifests.Conversions.AppsCreateFromManifestResult {
        val response = client.post("/app-manifests/$code/conversions")
        return when (response.status) {
            HttpStatusCode.Created -> AppManifests.Conversions.AppsCreateFromManifestResult.Created(response.body())
            HttpStatusCode.NotFound -> AppManifests.Conversions.AppsCreateFromManifestResult.NotFound(response.body())
            HttpStatusCode.UnprocessableEntity -> AppManifests.Conversions.AppsCreateFromManifestResult.UnprocessableEntity(response.body())
