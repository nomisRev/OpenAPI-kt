...
interface Installation {
    val repositories: Installation.Repositories

    val token: Installation.Token

    interface Repositories {
        @Serializable
        data class AppsListReposAccessibleToInstallationResponse(
        ...
        sealed interface AppsListReposAccessibleToInstallationResult {
            data class OK(val value: AppsListReposAccessibleToInstallationResponse) : AppsListReposAccessibleToInstallationResult

            data object NotModified : AppsListReposAccessibleToInstallationResult

            data class Unauthorized(val value: BasicError) : AppsListReposAccessibleToInstallationResult

            data class Forbidden(val value: BasicError) : AppsListReposAccessibleToInstallationResult
        }

        suspend fun appsListReposAccessibleToInstallation(
            page: Long = 1L,
            perPage: Long = 30L,
    ...
    interface Token {
        suspend fun appsRevokeInstallationAccessToken(): Unit
    }
}

internal class KtorInstallation(private val client: HttpClient) : Installation {
    override val repositories: Installation.Repositories = KtorInstallationRepositories(client)

    override val token: Installation.Token = KtorInstallationToken(client)
}

internal class KtorInstallationRepositories(private val client: HttpClient) : Installation.Repositories {
    override suspend fun appsListReposAccessibleToInstallation(page: Long, perPage: Long): Installation.Repositories.AppsListReposAccessibleToInstallationResult {
        val response = client.get("/installation/repositories") {
            parameter("page", page)
            parameter("per_page", perPage)
        }
        return when (response.status) {
            HttpStatusCode.OK -> Installation.Repositories.AppsListReposAccessibleToInstallationResult.OK(response.body())
            HttpStatusCode.NotModified -> Installation.Repositories.AppsListReposAccessibleToInstallationResult.NotModified
            HttpStatusCode.Unauthorized -> Installation.Repositories.AppsListReposAccessibleToInstallationResult.Unauthorized(response.body())
            HttpStatusCode.Forbidden -> Installation.Repositories.AppsListReposAccessibleToInstallationResult.Forbidden(response.body())
            else -> throw ResponseException(response, "Undocumented status code: ${response.status}")
        }
    }
}

internal class KtorInstallationToken(private val client: HttpClient) : Installation.Token {
    override suspend fun appsRevokeInstallationAccessToken(): Unit =
        client.delete("/installation/token").body()
}
